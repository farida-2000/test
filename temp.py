...
dfy=pd.read_csv('/media/data1/farida1/t/depression.csv')
for k in range(10):
    test_sub_y = df_trans[df_trans.fold == k].iloc[:, 0].tolist()
    df_test_y = dfy[dfy['ts_sub_id'].isin(test_sub_y)]
    train_sub_y = df_trans[df_trans.fold != k].iloc[:, 0].tolist()
    df_train_y = dfy[dfy['ts_sub_id'].isin(train_sub_y)]
    # print(df_test_y)
    # print(df_train_y)
    ...
    for i in range(len(df_train_x)):
        ...
        dfy = pd.read_csv("/media/data1/farida1/t/depression.csv",
                          usecols=['ts_sub_id', 'nac_gdstotal_6'])
        dfy = dfy.loc[dfy.ts_sub_id.isin([df_train_x[i]])]
        labels = dfy['nac_gdstotal_6'].values
        labels_int=labels[0]
        #labels_int = [int(j) for j in labels]
        if (labels_int=='0' or labels_int=='1' or labels_int=='2' or labels_int=='3' or labels_int=='4'):
            labels_int=0
        else:
            labels_int=1
    ...
    for i in range(len(df_test_x)):
        ...
        df_temp = dfy.loc[dfy.ts_sub_id.isin([df_test_x[i]])]
        ...
        dfy = pd.read_csv("/media/data1/farida1/t/depression.csv",
                          usecols=['ts_sub_id', 'nac_gdstotal_6'])
        dfy = dfy.loc[dfy.ts_sub_id.isin([df_test_x[i]])]
        labels = dfy['nac_gdstotal_6'].values
        labels_int = labels[0]
