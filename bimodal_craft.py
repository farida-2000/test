...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(768,6) #(768,6) for two modalilties (bimodal)
        self.fc2 = nn.Linear(6, 1)
        #self.fc3=nn.Softmax(1)
    def forward(self, x):
        x = F.relu(self.fc1(x))
...
#craft_fold0 is the first random set for folds. We repeat this experiment five other times with a different fold set (craft-fold-i) for generalization.
#df_trans=pd.read_csv('/media/data1/farida1/t/craft-hobbies/craft_foldi.csv')
df_trans=pd.read_csv('/media/data1/farida1/t/craft-hobbies/craft_fold0.csv')
dfy=pd.read_csv('/media/data1/farida1/t/Daytimetvshow/subjects_basic_info.csv')
...
for k in range(10):
    ...
    for i in range(len(df_train_x)):
        ...
        joint_rep = torch.cat((s,v))
        #joint_rep = s1.reshape(1, 768)
